...
uint16 IPS200_BGCOLOR=WHITE;
uint16 IPS200_PENCOLOR=RED;
//-------------------------------------------------------------------------------------------------------------------
//  @brief      内部调用，用户无需关心
//  @param      void 		    
//  @return     				
...
_Bool position_line(uint32 i,uint32 j,uint32 x,uint32 y)
{
    if((j==y&&i>x-12&&i<x+12)||(i==x&&j>y-10&&j<y+10))
    {
        if(!red)
    ...
    for(j=0;j<coord_y;j++)
    {
        for(i=0;i<coord_x;i++)
        {
            ...
            if(temp==5&&gray)
                ...
                red=0;
            if(position_line(i,j,26,35)){}
            else if(position_line(i,j,width/2,15)){}
            else if(position_line(i,j,width-26,35)){}
            else ips200_wr_data16(color);
