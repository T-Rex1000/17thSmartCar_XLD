...
uint16 TFT_PENCOLOR=RED;
uint16 TFT_BGCOLOR=WHITE;
...
uint8 mulitple=1;
unsigned char red=0;
_Bool point_TFT(uint32 i,uint32 j,uint32 x,uint32 y)
{
    x/=mulitple;y/=mulitple;
    if((j==y&&i>x-6&&i<x+6)||(i==x&&j>y-5&&j<y+5))
    {
        if(!red)
...
_Bool line_TFT(int i,int j,int x,int y,float k,int length)
{

    x/=mulitple;y/=mulitple;length/=mulitple;
    if(k==0)
    {
        if(i==x&&j<y&&j>y-length&&x>=0&&y-length>=0)
        {
            if(!red)
                lcd_writedata_16bit(GREEN);
            else
    ...
    {
        k=1/k;
        if((j-y)==k*(i-x)&&j<y&&j>y-length&&i>=0&&k*(i-x)+y>=0)
        {
            if(!red)
                lcd_writedata_16bit(GREEN);
            else
...
/*
int conline(int i,int j)
{
    if(j>=0&&j<60&&i<=imagezip[j]+1&&i>imagezip[j]-1)
    {
        if(!red)
            lcd_writedata_16bit(GREEN);
        else
...
*/
int oval_TFT(int i,int j)
{
    if(j<=55-1*(55)*pow(1-pow((i-45),2)/pow(45,2),0.5)+1&&j>=55-1*(55)*pow(1-pow((i-45),2)/pow(45,2),0.5)-1)
    {
        if(!red)
            lcd_writedata_16bit(GREEN);
        else
...
int arc_TFT(int i,int j,int x,int y,int r,int over_y)
{
    x/=2;y/=2;r/=2;over_y/=2;
    if((j==-1*(int)pow(r*r-(int)pow(i-x,2),0.5)+y)/*&&(j>=-1*pow(r*r-pow(i-x,2),0.5)+y-1)*/)
    {
        if(!red)
            lcd_writedata_16bit(GREEN);
        else
...
void lcd_displayimage032(int *p, uint16 width, uint16 height)
{
    uint32 i,j;
    ...
    {
        coord_x = (height<TFT_Y_MAX)&&(width<TFT_X_MAX)?width:width/2;
        coord_y = (height<TFT_Y_MAX)&&(width<TFT_X_MAX)?height:height/2;
        ...
        for(j=0;j<coord_y;j++)
        {
            ...
            for(i=0;i<coord_x;i++)
            {
                ...
                temp = *(p+mulitple*j*width+i*(width/coord_x));//读取像素点
...
//-------------------------------------------------------------------------------------------------------------------
void lcd_display_chinese(uint16 x, uint16 y, uint8 size, const uint8 *p, uint8 number, uint16 color, uint8 delay)
{
    int i, j, k; 
